  ...
  if (buttonType == "buttonrollreq") {
    ...
    if (flags.sw25.method == "skill") {
      ...
      if (flags.sw25.refAbility != "-") {
        let i18ncat =
          flags.sw25.refAbility.charAt(0).toUpperCase() + flags.sw25.refAbility.slice(1);
        let abi = " + " + game.i18n.localize(`SW25.Ability.${i18ncat}.abbr`);
        checkName = `${checkName}${abi}`;
      }
    }
    if (flags.sw25.method == "check") {
      if (itemData) {
    ...
    if (flags.sw25.checkName == "adv") formula = "2d6" + checkbase;
    ...
    if (checkName && !item && flags.sw25.checkName != "adv")
    ...
    if (flags.sw25.targetValue) {
      if (typeof flags.sw25.targetValue === "number") {
        ...
        targetValues = flags.sw25.targetValue.match(/[/,／， 　]/) 
         ? flags.sw25.targetValue.split(/[/,／， 　]/).map(v => parseInt(v, 10)).filter(v => !isNaN(v)) 
